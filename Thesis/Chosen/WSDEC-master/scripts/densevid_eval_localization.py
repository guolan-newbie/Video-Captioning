    ...
    def evaluate(self):
        ...
        if self.verbose:
            ...
            try:
                new_score = []
                for tiou in self.tious:
                    new_recall, _ = self.evaluate_new_detection(tiou)
                    new_score.append(new_recall)
                self.scores['R@1 score'] = new_score
            except:
                import traceback
                traceback.print_exc()


    def evaluate_new_detection(self, tiou):
        ...
        num_pred = 0
        assert len(self.ground_truths) == 1
        ...
        for vid_i, vid_id in enumerate(gt_vid_ids):
            ...
            if vid_id in self.prediction:
                for pred_i, pred in enumerate(self.prediction[vid_id]):
                    ...
                    gt_timestamp = pred['gt_timestamp']
                    if self.iou(pred_timestamp, gt_timestamp) > tiou:
                        ref_set_covered.add(pred_i)
                        pred_set_covered.add(pred_i)

                new_precision = float(len(pred_set_covered))
                new_recall = float(len(ref_set_covered)) / (pred_i + 1)
                num_pred += pred_i + 1
                recall[vid_i] = new_recall
                precision[vid_i] = new_precision
        return sum(precision) / num_pred, sum(recall) / len(recall)
...
def main(args):
    ...
    # Print the averages
    print '-' * 80
    print "Average across tIoUs=%s"%args.tious
    print '-' * 80
    for metric in evaluator.scores:
...
if __name__=='__main__':
    ...
    parser.add_argument('--tious', type=float,  nargs='+', default=[0.1, 0.3, 0.5, 0.7, 0.9],
