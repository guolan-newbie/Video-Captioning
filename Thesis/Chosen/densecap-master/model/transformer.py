...
class ResidualBlock(nn.Module):
    ...
    def forward(self, *x):
        ...
        # norm[x + f(x)]
"""
        *x : accept any number of paras, input as tuple
        **x : ......., input as dict
"""
...
class Attention(nn.Module):
    ...
    def __init__(self, d_key, drop_ratio, causal):
        super().__init__()
        self.scale = math.sqrt(d_key) #要除根号d的嘛
        self.dropout = nn.Dropout(drop_ratio)
        self.causal = causal  #

    def forward(self, query, key, value):
        dot_products = matmul(query, key.transpose(1, 2)) # query * key
        ...
        # softmax[query*key/sqrt(d)]*value
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x):
        return self.feedforward(self.selfattn(x, x, x)) # Self Attention + FC
...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, d_hidden, n_heads, drop_ratio):
        ...
        self.selfattn = ResidualBlock(
            ...
            d_model, drop_ratio)    #Self Attention

        self.attention = ResidualBlock(
            MultiHead(d_model, d_model, n_heads, drop_ratio),
            d_model, drop_ratio)   # encoder-decoder attention

        self.feedforward = ResidualBlock(FeedForward(d_model, d_hidden),
                                         d_model, drop_ratio)   #FC

    def forward(self, x, encoding):
        x = self.selfattn(x, x, x) # self attention
...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask=None):
        # x = self.linear(x)
        x = x+positional_encodings_like(x)  # plus pos-encoding
        x = self.dropout(x) 
        if mask is not None:
            x = x*mask    
        ...
        for layer in self.layers:
            x = layer(x)    #compute layer by layer
            if mask is not None:
                x = x*mask
            encoding.append(x)  #record codes for each layer
        return encoding
    """
                Mask is used in encoder..
                For Each proposal, the encoding stage will be done again..
                Mask is used to exclude the information outside of the proposed region..
    """
...
class Decoder(nn.Module):
    ...
    def forward(self, x, encoding):
        #接受一个隐层输入和视觉信息，做一次forward传播
        x = F.embedding(x, self.out.weight * math.sqrt(self.d_model))
        x = x+positional_encodings_like(x)
        x = self.dropout(x)
        # x : pre sequence
        # encoding : Encoder states

        for layer, enc in zip(self.layers, encoding):
            x = layer(x, enc)
        return x   #最后一个Decoder层的输出
                   #还需要经过一个Linear和Softmax计算概率分布

    def greedy(self, encoding, T):
        # 直接用Encoding开始caption
        B, _, H = encoding[0].size() # batch_size * length * feature_dimension
        ...
        prediction = Variable(encoding[0].data.new(B, T).long().fill_(
            self.vocab.stoi['<pad>']))           # batch * sentence_size

        hiddens = [Variable(encoding[0].data.new(B, T, H).zero_())
                   for l in range(len(self.layers) + 1)]
                                                # Layer_cnt * batch * sentence_size * dimension ?

        embedW = self.out.weight * math.sqrt(self.d_model)  

        hiddens[0] = hiddens[0] + positional_encodings_like(hiddens[0]) # 0传入。。

        for t in range(T):  # T words
            if t == 0:
                hiddens[0][:, t] = hiddens[0][:, t] + F.embedding(Variable(
                    encoding[0].data.new(B).long().fill_(
                        self.vocab.stoi['<init>'])), embedW)  #第一个词用<init>开头
            else:
            ...
            for l in range(len(self.layers)):  # L layers
                x = hiddens[l][:, :t + 1]    # 前t个hidden state
                x = self.layers[l].selfattn(hiddens[l][:, t], x, x)
                                            #self attention, use t as the query
        ...
        return hiddens, prediction # decoder 的 hiddens（embedding后），prediction（words ID）
    ...
    def sampling(self, encoding, gt_token, T, sample_prob, is_argmax=True):
        ...
        for t in range(T):
            if t == 0:
                ...
                # 根据随机摇奖：从上一个预测中取材 Or ground truth lable
...
class Transformer(nn.Module):  # only encode, for proposal maker
