...
def main(args):
    ...
    for train_epoch in range(args.max_epochs):
        ...
        ##################### train of current epoch #########################
        epoch_loss = train(train_epoch, model, optimizer, train_loader,
                           vis, vis_window, args)
        all_training_losses.append(epoch_loss)  # add training loss
        ######################################################################
...
def train(epoch, model, optimizer, train_loader, vis, vis_window, args):
    ...
    model.train()   # 将Training设置为true
    train_loss = [] 
    nbatches = len(train_loader)
    t_iter_start = time.time()

    sample_prob = min(args.sample_prob, int(epoch/5)*0.05)
    # 以一定概率从Model的输出中抽取context，默认为0，则全从GT中抽取。

    for train_iter, data in enumerate(train_loader):

        (img_batch, tempo_seg_pos, tempo_seg_neg, sentence_batch) = data
        # Sentence Batch from data
        img_batch = Variable(img_batch)  # img feature
        tempo_seg_pos = Variable(tempo_seg_pos) # positive seg
        tempo_seg_neg = Variable(tempo_seg_neg) # negative seg
        sentence_batch = Variable(sentence_batch) # GT Sentence ?
